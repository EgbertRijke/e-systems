...
\setbeamercolor{block title}{fg=teal!80!black}
...
\newcommand\goal[1]{\begin{huge}\begin{center}\textbf{Goal: {\color{teal}#1}}\end{center}\end{huge}}
...
\begin{frame}
\frametitle{\bf Current goal}
\goal{Define extension algebras}
\end{frame}
...
\begin{frame}
\frametitle{\bf Current goal}
\goal{Develop properties of extension algebras}
...
\begin{frame}
\frametitle{\bf Introduction to the theory of weakening}
\begin{itemize}
...
\begin{frame}
\frametitle{\bf Pre-weakening}
Let $\stesys$ be an extension algebra. A pre-weakening operation
...
A pre-weakening algebra $\stesys$ is an extension algebra $\stesys$ 
...
A pre-weakening homomorphism between pre-weakening algebras $\stesys'$ and $\stesys$ is an
...
Thus, it makes sense to require that the pre-weakening operation itself is a
pre-weakening homomorphism.
\begin{definition}
A \emph{weakening algebra} is a pre-weakening algebra $\stesys$ with the property that
...
\begin{definition}
A projection algebra is a pre-projection algebra for which the weakening operation
...
commutes.\\
\pause
A \emph{pre-substitution algebra} is an extension algebra
...
\begin{definition}
A \emph{substitution algebra} is a pre-substitution algebra $\stesys$ with the property that
...
\item that weakening by a family $A$ followed by substitution of the unit of $A$ has no effect; this is precomposing with the unit.
\end{itemize}
\end{frame}

\begin{frame}
\frametitle{\bf Pre-E-systems}
A \emph{pre-E-system} is an extension algebra $\stesys$ with a
...
\end{itemize}
Pre-E-homomorphisms are extension homomorphisms which
...
In a pre-E-system, we say that \emph{weakened families are 
...
In a pre-E-system $\stesys$ we say that \emph{everything is invariant
