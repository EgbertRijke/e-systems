...
\begin{defn}\label{defn:CEsys}
An \define{CE-system} consists of a pre-category $\cat{C}$ and a subpre-category $\cat{F}$ with
...
\begin{thm}
Every CE-system is an E-system.
\end{thm}
...
In this section we define for every E-system $\mathbb{E}$, and every context $\Gamma$ in $\mathbb{E}$, a category $\mathcal{C}_\mathbb{E}(\Gamma)$. This goal is accomplished in \autoref{thm:internal_hom}. Note that we do not have a category structure on the set of contexts of $\mathbb{E}$, unless $\mathbb{E}$ possesses an empty context $[]$. In this case, the category structure is inherited from the category $\mathcal{C}_{\mathbb{E}}([])$.
...
\begin{thm}\label{thm:internal_hom}
For each object $\Gamma$ of a E-system $\mathbb{E}$, we have a pre-category
$\mathcal{C}_{\mathbb{E}}(\Gamma)$ of internal morphisms of $\mathbb{E}$,
...
$f^\ast$, i.e.~for any $f\in\thom{A}{B}$ in $\mathbb{E}$ and any E-homomorphism
$F:\mathbb{E}\to\mathbb{D}$, the diagram
\begin{equation*}
\begin{tikzcd}[column sep=large]
\mathbb{E}/\ctxext{\Gamma}{A} \arrow[r,"F/\ctxext{\Gamma}{A}"] & \mathbb{D}/F(\ctxext{\Gamma}{A})
  \\
\mathbb{E}/\ctxext{\Gamma}{B} \arrow[u,"f^\ast"] \arrow[r,swap,"F/\ctxext{\Gamma}{B}"] & \mathbb{D}/F(\ctxext{\Gamma}{B}) \arrow[u,swap,"F(f)^\ast"]
\end{tikzcd}
\end{equation*}
commutes. This applies to weakening, substitution and pre-composition, and it
follows that for any E-homomorphism $F:\mathbb{E}\to\mathbb{D}$ and any
$\Gamma\in\mathbb{E}$, we have a functor $\mathcal{C}_F(\Gamma):
\mathcal{C}_{\mathbb{E}}(\Gamma)\to\mathcal{C}_{\mathbb{D}}(F(\Gamma))$.
\end{rmk}
...
Let $F:\mathbb{E}\to\mathbb{D}$ be an E-homomorphism and let $x\in T(A)$ and
...
\begin{equation*}
F^\bullet\defeq F^\ast\circ (f^\ast/Q):
\mathbb{E}/\ctxext{{\Gamma}{B}}{Q}\to\mathbb{E}/\ctxext{{\Gamma}{A}}{P}.
\end{equation*}
The infix notation of $F^\bullet$ is taken to be $\jfcomp{\blank}{\blank}{\blank}{F}{\blank}$.
\end{defn}
...
have the equality
\begin{equation*}
F^\bullet\jdeq (\jvcomp{P}{f}{F})^\ast.
\end{equation*}
\end{lem}
...
In the next theorem we prove the interchange law of horizontal and vertical composition.
Its proof uses the following fact.
...
\end{equation*}
in context $\Gamma$. 
Then $F^\bullet\circ G^\bullet\jdeq (\jfcomp{A}{f}{P}{F}{G})^\bullet$.
In other words the composition $\jfcomp{}{}{}{\blank}{\blank}$ is associative.
\end{thm}

\begin{proof}
\begin{align*}
F^\bullet\circ G^\bullet
  & \jdeq
...
\begin{rmk}
When $\mathbb{E}$ is a E-system with empty context, then we have the category
structure on the contexts given by $\thom{\Gamma}{\Delta}= T(W_\Gamma(\Delta))$.
\end{rmk}

\begin{defn}
A \emph{natural transformation} of E-homomorphisms from $F:\mathbb{E}\to\mathbb{D}$
to $G:\mathbb{E}\to\mathbb{D}$ preserving the empty context consists of
\begin{enumerate}
\item A t-morphism $\tau0_A\in\thom{F(\Gamma)}{G(\Gamma)}$ for every $\Gamma\in\mathbb{E}$.
...
\end{tikzcd}
\end{equation*}
commutes for every $A,B\in\mathbb{E}/\Gamma$. Note that 
\begin{align*}
W_{F(A)}(\tau1_B)
  & \in 
T(W_{F(A)}(W_{F(B)}(G(B))))
...
\begin{lem}
For each $A\in\mathbb{E}/\Gamma$, $W_A$ is left adjoint to $\Pi_A$. 
\end{lem}
