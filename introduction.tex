...
The theory of telescopic type systems is a theory of type dependency in which we have a notion of family extension. They are called such because if we try to interpret a category with families as a telescopic type systems, one of the things we have to do is to consider lists of dependent types as our notion of families. These are the objects De Bruijn called telescopes. On the other hand, the theory of telescopic type systems is an (essentially) algebraic theory of the notion of structure. A family over a context $\Gamma$ in a telescopic type system is a structure in context $\Gamma$. Therefore I am also considering the name `structure algebras'. Types are a defined notion in telescopic type systems. They are the atomic families (or structures), i.e.~the ones which do not decompose as the extension of two non-trivial structures. 

\subsection*{Setting up the essentially algebraic theory of telescopic type systems}
The theory of telescopic type systems we present in the present article is an iteration in the
...
a telescopic type system is therefore an algebra consisting of three sorts: $C$ for contexts,
...
which family it is a term. Thus, the backbones of a telescopic type system can be represented
as
\begin{equation*}
\begin{tikzcd}
...
One may recognize in these ingredients of telescopic type systems a category with a
...
Thus, a telescopic type system $\mathbf{E}$ will have an underlying category, which may be called the
...
stratified telescopic type systems. However, we do not arrive at the fact that the functor
$\mathcal{E}:\mathbf{Bsys}\to\mathbf{Tel}$ is full without imposing an additional
condition on the E-homomorphisms. 
To ensure that the category of stratified telescopic type systems with stratified 
E-homomorphisms between them is a full subcategory of the category 
$\mathbf{Tel}$, we will require that the underlying functor of each E-homomorphism
lifts factorizations. It is
important to note that this condition is also algebraic, so that the category
of telescopic type systems remains algebraic.
...
\subsection*{The pre-categorical nature of telescopic type systems}
Although we start our definition of telescopic type systems with a category with term structure,
we do not set out to have a categorical notion of telescopic type systems, in the sense that the structure of
being a telescopic type system can be transported along equivalences of categories. The goal
...
of our definition of telescopic type systems is in a sense coincidence, rather than born out
of a desire to do type theory categorically and we would consider telescopic type systems to
...
functor from categories with families to telescopic type systems will forget that the contexts
...
telescopic type systems are like contextual categories in this respect: only a singleton can
be a telescopic type system with no types. This complicates a thorough comparison between
categories with families and comprehension categories on one side, and telescopic type systems
...
In \autoref{sec:esys_defn}, we give the categorical definition of telescopic type systems in 
...
of telescopic type systems. The structure of the definition of weakening systems is 
...
of weakening systems, projection systems, substitution systems and telescopic type systems.
...
E'-systems and telescopic type systems are different axiomatizations of the same structures,
and apart from that the goal is to show some equational reasoning in telescopic type systems.
...
of telescopic type systems is the one where E-homomorphisms lift factorizations. The condition
that a functor lifts factorizations is also algebraic, so we still have an
essentially algebraic theory of telescopic type systems if we require this.
In \autoref{sec:esys_compared}, where we introduce the category of telescopic type systems
with E-homomorphisms lifting factorizations, we construct an embedding from Voevodsky's
B-systems into telescopic type systems and prove that its image is the (full) subcategory
of stratified telescopic type systems. 
...
the more convenient symbol $=$ for the internal equality of telescopic type systems, i.e.~for
...
over a context -- which will be the outset of the definition of telescopic type systems --
...
for instance), telescopic type systems can actually be defined with finitely many
operations. This becomes clearer when one defines a telescopic type system in an arbitrary
...
definition of a telescopic type system in arbitrary categories with finite limits;
and Vladimir Voevodsky for inviting me to present telescopic type systems at the Homotopy Type
