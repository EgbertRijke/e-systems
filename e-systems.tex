...
\begin{defn}
A \define{non-unital pre-E-system} is a pre-category $\cat{F}$ with term structure
with a pre-weakening structure $W$, a pre-substitution structure $S$ and a terminal
object. A \define{unital pre-E-system}, or simply \define{pre-E-system}, is a non-unital pre-E-system which also has the structure of a pre-projection system. 
\end{defn}

\begin{defn}\label{defn:E0sys_cat}
...
We write $\mathbf{E0sys}$ for the category of pre-E-systems. 
\end{defn}
...
\begin{cor}
For any object $\Gamma$ of a E-system $\cat{F}$, the slice pre-category $\cat{F}/\Gamma$
is a E-system.
\end{cor}

\begin{defn}\label{defn:esys_cat}
...
each occurrence of $S_x\circ W_A$ and $S_{\tfid{A}}\circ W_A/A$ is eliminated.
\end{rmk}
\end{comment}
