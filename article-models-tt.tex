...
Only a family of contexts over a context $\Gamma$ is eligible to
...
under this category. However, we will primarily investigate families of
contexts and leave discussion on the possibility of a typing judgment until the
end of this section. With families of contexts being the principal things of
study, we have added a bit more generality to the theory, compared to the
theory of categories with families presented in \cite{Dybjer1996}. The reason
for this added generality is twofold:
\begin{enumerate}
...
\subsubsection{The empty families}
\label{empty}
We introduce an empty family over $\Gamma$ for every context $\Gamma$. It was
a suggestion of Voevodsky and Garner to only include empty families and not an
empty context. Having an empty context requires also rules asserting that a
context is the same thing as a family over the empty context and that operations
act the same on families over the empty context as on contexts. Not including
an empty context changes the theory a bit. One example of a difference is that
if we want weakening by a context we shall have to explicitly state it as it 
would not follow from weakening by a family. A second example is that without
an empty context, some rule schemes (like the compatibility of extension with
the empty families, stated in \autoref{comp-0e} below) become slightly
assymetrical. Another example is that it would
not be possible to consider closed types and terms without an empty context.
Although we have opted to not include an empty context by default, we
shall give the rules in \autoref{empty-context} so that one can look them up
if they are desired.

{\color{red}Explain why empty families are needed}
...
\end{equation}
Since it is not really necessary to annotate the empty family with its context,
we shall usually denote $\emptyf[\Gamma]$ by $\emptyf$.
