\section{E-systems compared to Voevodsky's B-systems}\label{sec:esys_compared}
...
\begin{defn}
A \define{non-unital B0-system} $\mathbb{B}$ consists of a B-framework $\mathbb{B}$ and homomorphisms
\begin{align*}
...
\begin{defn}
A \define{non-unital B0-homomorphism} $H:\mathbb{B}\to\mathbb{A}$ is a homomorphism of B-frameworks
...
\begin{defn}
A \define{(unital) B-system} is a B0-system for which the following conditions
hold:
\begin{enumerate}
...
\catid{\mathbb{B}/X}$. 
\end{enumerate}
\define{B-homomorphisms} are unital B0-homomorphisms between B-systems.
...
There is essentially only one choice for the presheaf $\mathrm{Tm}$ and the term $\delta_A$, satisfying the universal property described in the definition of categories with families.
...
\begin{defn}
A \define{category with dependent contexts} consists of
\begin{enumerate}
...
The following two definitions define subclasses of \emph{telescopic} categories with families and 
...
\begin{comment}
\begin{lem}[Work in progress]
A category with families is telescopic if and only if  the following two conditions hold:
\begin{enumerate}
...
\begin{proof}[Work in progress]
Let $\mathbb{D}$ be a telescopic category with families. To show that $\mathbb{D}$ has the two
stated properties:
\begin{enumerate}
...
Let $\mathbb{D}$ be a category with families for which the stated two conditions hold. To show
that $\mathbb{D}$ is a telescopic category with families, we systematically go through each of the
properties:
\begin{enumerate}
\item 
\end{enumerate} 
...
\begin{defn}
A category with dependent contexts is said to be \define{coherent}, if for any $A,A'\in\mathrm{Fam}(\Gamma)$
such that $p_A\jdeq p_{A'}$, and any $f:\Delta\to\Gamma$, we have
$p_{f^\ast(A)}\jdeq p_{f^\ast(A')}$ and $\pi_2(f,A)\jdeq \pi_2(f,A')$.
\end{defn}

\begin{defn}
A category with dependent contexts is said to be \define{separated}, if for any $A,A'\in\mathrm{Fam}(\Gamma)$, we have
\begin{equation*}
...
To get from comprehension categories to CE-systems without empty contexts, we
...
\begin{defn}[To do: transport the notion of telescopic from CWFs to D-systems (probably by reading the proof below)]\label{defn:comp_to_telescopic}
