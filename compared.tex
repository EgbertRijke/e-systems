\section{Context algebras compared to Voevodsky's B-systems}\label{sec:esys_compared}
...
\subsection{Stratified context algebras}

In this subsection we define the notion of stratified context algebras. Then we show that the category $\mathbf{Tel}_s$ of stratified context algebras with homomorphisms preserving the stratification, is a full subcategory of $\mathbf{Tel}$. 
...
\begin{defn}
A pre-Context algebras is said to be \define{stratified} if its underlying category is
stratified and if each $W_A$ and $S_x$ is a stratified functor. The category of
stratified pre-Context algebras with stratified E-homomorphisms between them is denoted by
$\mathbf{Tel}_s$.
\end{defn}

\begin{cor}\label{lem:strat_full}
The category of stratified pre-Context algebras is a full subcategory of the category of
pre-Context algebras.
\end{cor}

\subsection{The embedding of B0-systems into pre-Context algebras}
...
Context algebras by forgetting the projection structure, but we have not been able to construct a
functor from unital B0-systems to unital pre-Context algebras. Therefore, we shall only
...
functor from them to the category of pre-Context algebras. 
...
many operations. Since we are interested in embedding the B-systems into context algebras,
...
embedding into pre-Context algebras.
...
of a pre-Context algebras. We write $\mathcal{E}(\mathbb{B})$ for the resulting
pre-Context algebras. Thus we get a functor $\mathcal{E}:\mathbf{B0sys}\to\mathbf{Tel}_s$.
\end{defn}
...
\subsection{B-systems are stratified context algebras}
There are two properties which hold for pre-Context algebras of the form $\mathcal{E}
(\mathbb{B})$ for some B0-system $\mathbb{B}$, 
which stand in the way of proving that the category of stratified
pre-Context algebras is equivalent to the category of B0-systems. The first is that
...
However, to prove the corresponding facts for context algebras, we needed the full
set of conditions on unital context algebras: to prove that the term structure of
...
Presumably, we can assume these compatibility conditions on pre-Context algebras in order
to prove the desired equivalence of B0-systems and stratified pre-Context algebras. However,
that seems somewhat unnatural, especially given that they automatically hold
for context algebras. Thus, for the remainder of this section we set out to show that
the category of unital B-systems is equivalent to the category of stratified 
unital context algebras.
...
stratified non-unital context algebras. 
\end{lem}
...
$\mathcal{E}:\mathbf{Bsys}\to\mathbf{Tel}$ from unital B-systems to context algebras.
\end{defn}

\begin{constr}
We first need to extend the functor $\mathcal{E}:\mathbf{B0sys}\to\mathbf{Esys_p}$
from non-unital B0-systems to non-unital pre-Context algebras, to a functor from
unital B-systems to context algebras.

We need to give $\mathcal{E}(\mathbb{B})$ the structure of a unital
pre-Context algebras. We construct by induction on $m\in \mathbb{N}$, a term of type 
...
to the subcategory $\mathbf{Tel}_s$ of stratified context algebras: i.e.~we have a
commuting diagram
\begin{equation*}
\begin{tikzcd}
...
Context algebras and that it is full and faithful, so it remains
to show that every stratified context algebras is in the image of $\mathcal{E}$. In
other words, for every stratified context algebras $\mathbb{E}$ we need to construct
...
homomorphisms of B-frameworks. Because a weakening system of a context algebras is
...
\section{Context algebras compared to comprehension categories and to categories with families}
...
with families to the category of context algebras. 
...
We could only hope that there is a functor from D-sytems to context algebras without
terminal objects.
\end{rmk}
...
Context algebras with various existing notions of models of type dependency, it suffice
...
Context algebras, even if we ignore the issue that categories with families map
naturally into E'-systems without empty context.
Thus it seems that the type theories of categories with families and of context algebras
...
Context algebras factors through D-systems. 
...
morphisms the context algebras $\mathcal{E}/\Gamma$. 
